...
def text_output(text):
    with open('V:\\Uni 3\\Project\\params.txt','a') as result_file:
...
def estimate(test_videos, test, rbf_svc):
    ...
    for video in test_videos:
        ...
        #Debugging override
        override_test = [True for x in range(13)]
        p = multiprocessing.Process(target=processVideo, args=(file, load_folder, override_test, stats_return_dict))
        threads.append(p)
        p.start()
    ...
    for value in stats_return_dict.values():
        ...
        for item_index in range(len(test)):
            if test[item_index]:
                test_features[-1].append(value[item_index+1])

        estimate = rbf_svc.predict([test_features[-1]])
        actual = value[0]
...
if __name__ == '__main__':
    ...
    for set_index in range(len(test_video_sets)):
        ...
        for video in training_video_sets[set_index]:
            ...
            for file in os.listdir(load_folder):
                ...
                if file_no % 2 == 0:
                    ...
                    p = multiprocessing.Process(target=processVideo, args=(file, load_folder, [True for x in range(13)], return_dict))
                    threads.append(p)
                    p.start()
        ...
        #grid.fit(data_list, category_list)

        print("The best parameters are %s with a score of %0.2f"
            % (grid.best_params_, grid.best_score_))
        text_output(str(grid.best_params_) +", "+ str(grid.best_score_))
        '''
        ...
        procedure.insert(0,[True for x in range(13)])
        test_accuracy = [[] for x in range(13)]
        test_difference = [[] for x in range(13)]
        for test_index in range(len(procedure)):
            data_list_copy = []
            for item in data_list:
                data_list_copy.append([])
                for index in range(len(procedure[test_index])):
                    if procedure[test_index][index]:
                        data_list_copy[-1].append(item[index])
                        
            rbfs.append(svm.SVC(kernel='rbf'))
            rbfs[-1].fit(data_list_copy, category_list)
            print("THE BIT YOU WANT\n\n\n\n\n\n")
            print(data_list_copy[0])

            exact_accuracy, lenient_accuracy, differences = estimate(test_video_sets[set_index], procedure[test_index], rbfs[-1])
            set_exact_accuracy = (exact_accuracy/total_test_videos) * 100
            set_lenient_accuracy = (lenient_accuracy/total_test_videos) * 100

            test_accuracy[test_index].append(set_exact_accuracy)
            test_difference[test_index].append(sum(differences))
            ...
            output_string = "\nExact Accuracy for test "+str(procedure[test_index]) + ", trial " +str(set_index)+": "+str(set_exact_accuracy)+"\n"
        ...
        break
        '''
    for test in test_accuracy:
        average_exact_accuracy = sum(test)/len(test)
