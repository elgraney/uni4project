...
def processVideo(file, load_folder, return_dict, debugging):
    ...
    for flow_file in os.listdir(file_path):
        flow_list.append(np.load(os.path.join(file_path, flow_file)))
    ...
    for frame_index in range(len(flow_list)):
        ...
        for flow_index in range(0, len((flow_list[frame_index])), step):
            # Optional ignore low values || REQUIRED TO AVOID DIVIDE BY ZERO
            magnitude = float(np.linalg.norm(flow_list[frame_index][flow_index]))
            if magnitude > 0.001:
    ...
    video_features["median"] = mean_feature(median_grouped_list)
...
def evalSet(set_results, video_set, directory):
    ...
    for video in video_set:
        ...
        load_folder = os.path.join(save_folder, "PointsFlow")
        ...
        count +=1 
        if count % round(total_folders/100) == 0:
...
if __name__ == '__main__':
    ...
    start = time.time()
    directory = "V:\\Uni 3\\Project\\OpticalFlow\\"
    Save_directory = "V:\\Uni 3\\Project\\DataSets\\PointsFlow\\"
    ...
    for set_index in range(len(training_video_sets)):
        ...
        training_features = evalSet(features_copy, training_video_sets[set_index], directory)

        features_copy = copy.deepcopy(features)
        test_features = evalSet(features_copy, test_video_sets[set_index], directory)


        if not os.path.exists(Save_directory+str(set_index)):
            os.mkdir(Save_directory+str(set_index))
        print("\n\nSaving")
        with open(Save_directory+str(set_index)+"\\TrainingSet"+str(set_index), 'w') as out:
            json.dump(training_features, out)
        with open(Save_directory+str(set_index)+"\\TestSet"+str(set_index), 'w') as out:
            json.dump(test_features, out)
